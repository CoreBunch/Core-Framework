import { Size, TypeScale, TypographyItem } from "../types";
import { round } from "utils";

interface IGetTypeScale {
	readonly state: TypographyItem;
...
export function getTypeScale({ state, minScreenWidth, maxScreenWidth }: IGetTypeScale): TypeScale[] {
	...
	for (const i of scaleArray) {
		...
		let min: Size = {
			fontSize: Number(state.min.fontSize) * Math.pow(Number(minScaleRatio), i),
			breakpoint: minScreenWidth,
		};

		let max: Size = {
			fontSize: Number(state.max.fontSize) * Math.pow(Number(maxScaleRatio), i),
