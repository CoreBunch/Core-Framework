...
interface IVariableRow {
	...
	readonly suggestionProperty: string;
	readonly isMoveDownDisabled: boolean;
	readonly disableColorInput?: boolean;
}

export const VariableRowWithAutoComplete = memo<IVariableRow>((variableRowProps) => {
	...
	const { refreshSuggestions, valueSuggestions } = useAutocomplete();
	...
	useEffect(() => {
		refreshSuggestions(variableRowProps.suggestionProperty);
	}, [refreshSuggestions, variableRowProps.suggestionProperty]);
...
VariableRowWithAutoComplete.displayName = "VariableRow";
