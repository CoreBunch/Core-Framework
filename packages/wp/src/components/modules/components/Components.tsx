...
export function Components() {
	...
	const menuItems = useMemo(
		...
		[
			JSON.stringify(componentsData?.isDisabled),
			JSON.stringify(componentsData?.isDisabled),
	...
	const handleImportSubmit = async () => {
		try {
			// IMPORTANT DIFFERENT IN PLUGIN
			const compressedString = (await navigator?.clipboard?.readText()).trim();
			...
			{componentsData?.isDisabled ? (
				...
				<>
					...
					<Modal
						...
						target={(document.getElementById("core-framework-init") ?? document.body) as HTMLElement}
						opened={isModalOpen}
						transitionProps={{
							transition: "fade",
							duration: 200,
						...
						padding={0}
						size="min(max(100vw - 3rem, 300px), 70rem)"
					>
						{selectedComponent ? (
							<ComponentsEditor
