...
export const ColorModule = memo(() => {
	...
	const handleColorGroupExport = useCallback(
		...
		[createPartialExport, copyToClipboard],
	);
	const handleColorGroupImport = useCallback(
		...
		[readClipboard, parsePartialImport, state, dispatch],
	);
	...
	const handleColorUpdate = useCallback(
		(value: ColorItem, groupId: string, withStateUpdate?: boolean) => {
			const base = stateRef.current ?? state; // <— latest snapshot, not the stale closure

			const groupIdx = base.groups.findIndex((g) => g.id === groupId);
			if (groupIdx === -1) return;

			const group = base.groups[groupIdx];
			const colors = group.colors || [];
			const colorIdx = colors.findIndex((c) => c.id === value.id);
			if (colorIdx === -1) return;

			const nextGroups = base.groups.map((g, i) =>
				i === groupIdx
					? {
							...g,
							colors: [...colors.slice(0, colorIdx), value, ...colors.slice(colorIdx + 1)],
					  }
					: g,
			);
			const nextState = { ...base, groups: nextGroups };

			stateRef.current = nextState;
			dispatch({ type: "restore", payload: nextState });

			if (withStateUpdate) setColorSystem(nextState);
		},
		[dispatch, setColorSystem],
	);
