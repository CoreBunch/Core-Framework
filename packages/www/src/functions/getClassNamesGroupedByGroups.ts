...
export const getClassNamesGroupedByGroups = (preset: Preset) => {
	const classNames: Record<Exclude<StylesheetDataKeys, "stylesheetsStyles">, Record<string, string[]>> = {
		colorStyles: {},
		typographyStyles: {},
		spacingStyles: {},
		layoutsStyles: {},
	...
	if (fluidSpacingObjects && Array.isArray(fluidSpacingObjects)) {
		...
		for (const className of uniqueFluidSpacingClasses) {
			if (className.includes(gapGroup[1])) {
				gapsClasses.push(className);
			}

			if (className.includes(paddingsGroupName[1])) {
				paddingsClasses.push(className);
			}

			if (className.includes(marginsGroupName[1])) {
